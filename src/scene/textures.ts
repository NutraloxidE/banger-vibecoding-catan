...
export function portSignTexture(rate: number, emoji: string): THREE.CanvasTexture {
  ...
  const r = 28;
  ctx.beginPath();
  ctx.moveTo(r, 12); ctx.arcTo(244, 12, 244, 244, r); ctx.arcTo(244, 244, 12, 244, r);
  ctx.arcTo(12, 244, 12, 12, r); ctx.arcTo(12, 12, 244, 12, r); ctx.closePath();
  ctx.fill();
  ctx.lineWidth = 12; ctx.strokeStyle = '#a9884f'; ctx.stroke();
  ctx.fillStyle = '#2f2413';
  ctx.font = 'bold 104px Georgia, serif';
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.fillText(`${rate}:1`, 128, 84);
  ctx.font = '100px system-ui, sans-serif';
  ctx.fillText(emoji, 128, 184);
  const tex = new THREE.CanvasTexture(c);
  tex.anisotropy = 4;
